...
* Emma Jones (emma.jones@deq.virginia.gov)
* Joe Famularo (joseph.famularo@deq.virginia.gov)
* Reid Downer (horace.downer@deq.virginia.gov)
* Kristen Bretz (kristen.bretz@deq.virginia.gov)
...
<!--chapter:end:dataOrganization.Rmd-->
...
A maximum temperature criteria applies to stations depending on their [associated WQS](https://law.lis.virginia.gov/admincode/title9/agency25/chapter260/section50/){target='_blank'}. The `tempExceedances()` function identifies any temperature exceedances of the `Max Temperature (C)` field for each sample provided. The function removes all Level I and Level II data and missing data before rounding measure to even and comparing against the provided criteria. An internal `7Q10 Flag` is passed through this function for use in the `quickStats()` function should a user adjust the default `drop7Q10` argument.
...
There are two general dissolved oxygen (DO) criteria that may apply to stations, depending on their [associated WQS](https://law.lis.virginia.gov/admincode/title9/agency25/chapter260/section50/){target="_blank"}. The minimum DO criteria are evaluated against the provided `Dissolved Oxygen Min (mg/L)` using the `DOExceedances_Min()` function. This function handles lake stations differently than other waterbody types. If a station is in a [Section 187 lake](https://law.lis.virginia.gov/admincode/title9/agency25/chapter260/section187/){target="_blank"}, then only the epilimnion and unstratified samples are evaluated. Regardless of the waterbody type, all Level I and Level II data and missing data are removed before measures are rounded to even and compared against the provided criteria. An internal `7Q10 Flag` is passed through this function for use in the `quickStats()` function should a user adjust the default `drop7Q10` argument.
...
A separate function called `DO_Assessment_DailyAvg()` evaluates whether or not a station exceeds the DO daily average criteria, provided in the `Dissolved Oxygen Daily Avg (mg/L)` field. This analysis is only run if the number of samples for a station in a given date are greater than one. This function handles lake stations differently than other waterbody types. If a station is in a [Section 187 lake](https://law.lis.virginia.gov/admincode/title9/agency25/chapter260/section187/){target="_blank"}, then only the epilimnion and unstratified samples are evaluated. Regardless of the waterbody type, all Level I and Level II data and missing data are removed before measures are rounded to even and compared against the provided criteria. An internal `7Q10 Flag` is passed through this function for use in the `quickStats()` function should a user adjust the default `drop7Q10` argument.
...
```{r automated assessment nutrients, eval=FALSE}
```
...
For a general flag provided to the Station Table to determine whether metals data do/do not exist, please see [PCB](#pcb) above. When data exists for these sediment or water column metals parameters, regional assessment staff may investigate them against applicable thresholds inside the waterbody-specific shiny assessment application. 
...
### Additional Automated Functions
...
<!--chapter:end:automatedAssessment.Rmd-->

# Regional Metadata Validation <br>Tool How To
...
## Application Use: <br>Water Quality Standards QA- <br>Watershed Selection Tab
...
## Application Use: <br>Water Quality Standards QA- <br>Manual Review Tab
...
### Application Operation: <br>Station that Snapped to 1 WQS Segment
...
### Application Operation: <br>Station that Snapped to 0 WQS Segments
...
### Application Operation: <br>There are no more stations to attribute
...
Please contact Emma Jones (emma.jones@deq.virginia.gov) should you encounter any problems using the application, wonky situations to review, or long rendering times/app crashing. 

<!--chapter:end:metadataApplication.Rmd-->
...
<!--chapter:end:riverineApplication.Rmd-->
...
<!--chapter:end:lacustrineApplication.Rmd-->
